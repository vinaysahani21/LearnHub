...
// Now it is safe to destructure because dashboardData is guaranteed to exist
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {/* 1. WELCOME BANNER */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 dark:border-slate-800/50 pb-6">
        <div className="w-full md:w-auto">
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3 break-words">
        ...
        <Link 
          to="/student/explore" 
          className="w-full md:w-auto justify-center bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-500/20 px-5 py-3 md:py-2.5 rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2 transition-all border border-sky-100 dark:border-sky-500/20"
        >
      ...
      {heroCourse ? (
        <div className="relative overflow-hidden bg-gradient-to-br from-[#0a0f1c] to-slate-900 rounded-3xl shadow-2xl border border-slate-800 text-white group">
          ...
          <div className="relative z-10 p-6 sm:p-8 md:p-12 flex flex-col-reverse md:flex-row gap-8 md:gap-10 items-center">
            ...
            <div className="flex-1 space-y-6 w-full">
              ...
              <div>
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-black leading-tight tracking-tight mb-3 group-hover:text-sky-400 transition-colors">
                  {heroCourse.title}
                </h2>
              ...
              <div className="pt-2">
                <button 
                  ...
                  className="w-full sm:w-auto bg-white text-slate-900 hover:bg-sky-50 px-8 py-3.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-lg active:scale-95"
                >
        ...
        /* EMPTY STATE HERO */
        <div className="bg-gradient-to-br from-sky-600 to-indigo-700 rounded-3xl p-8 md:p-12 text-white text-center shadow-2xl relative overflow-hidden">
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
          <div className="relative z-10">
            <Sparkles className="w-12 h-12 text-sky-200 mx-auto mb-6" />
            <h2 className="text-2xl sm:text-3xl font-black mb-4 tracking-tight">Your Canvas is Blank</h2>
            <p className="text-sky-100 mb-8 max-w-md mx-auto font-medium text-sm sm:text-base">
              You aren't enrolled in any courses yet. Explore our catalog of masterclasses and start building your future.
            </p>
            <Link to="/student/explore" className="w-full sm:w-auto justify-center bg-white text-sky-900 px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-sky-50 transition-all inline-flex items-center gap-2 shadow-xl shadow-sky-900/20 active:scale-95">
              Explore Catalog <ArrowRight size={16} />
            </Link>
          </div>
        </div>
...
const StatCard = ({ icon, label, value, color }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-sm flex items-center gap-4 sm:gap-5 hover:shadow-md transition-all group">
      <div className={`p-3.5 sm:p-4 rounded-2xl ring-1 transition-transform group-hover:scale-110 shrink-0 ${colors[color]}`}>
        {icon}
      </div>
      <div className="min-w-0">
        <p className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none truncate">{value}</p>
        <p className="text-[9px] sm:text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black mt-1.5 truncate">{label}</p>
      </div>
    </div>
  );
};
