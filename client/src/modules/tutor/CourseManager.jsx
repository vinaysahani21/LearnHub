...
const CourseManager = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 relative">
      ...
      {/* 1. STUDIO HEADER */}
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 transition-colors">
        ...
        <div className="flex-1 w-full">
          ...
          <div className="flex items-center gap-5">
            <div className="relative shrink-0">
              <img src={course.thumbnail} className="w-24 h-16 object-cover rounded-xl shadow-md border border-slate-100 dark:border-slate-700" alt="Thumbnail" />
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight line-clamp-1">{course.title}</h1>
            </div>
          </div>
        </div>

        {/* Right Side: ACTION CONTROLS */}
        <div className="flex items-center gap-2 w-full md:w-auto bg-slate-50 dark:bg-slate-800/50 p-2 rounded-2xl border border-slate-100 dark:border-slate-700/50">
          ...
          {/* 🔥 PREVIEW BUTTON: Routed to the upcoming Tutor Preview page! */}
          ...
          <button 
            ...
            className={`p-2.5 rounded-xl transition-all shadow-sm flex items-center gap-2 text-xs font-black uppercase tracking-wider ${
              course.isActive 
          ...
          <Link to={`/tutor/course/${course._id}/add-lesson`} className="ml-1 px-5 py-2.5 bg-[#0a0f1c] dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-500 text-white rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-slate-900/10 dark:shadow-indigo-900/20 transition-all active:scale-95">
            <Plus size={16} /> Module
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-xl shadow-slate-200/10 dark:shadow-none overflow-hidden transition-colors">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
          <div>
            <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Curriculum Planner</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Build, arrange, and manage course content.</p>
          </div>
        </div>

        <div className="divide-y divide-slate-100 dark:divide-slate-800/50 p-4">
          {course.lessons?.length === 0 ? (
            <div className="p-16 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl m-4 bg-slate-50/50 dark:bg-slate-800/20">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-bold mb-4">No content has been uploaded yet.</p>
            ...
            course.lessons?.map((lesson, index) => (
              <div key={lesson._id || index} className="p-4 flex items-center gap-4 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-2xl border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all group mb-2">
                <div className="cursor-grab active:cursor-grabbing text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400">
                   <GripVertical size={20} />
                </div>
                <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-black flex items-center justify-center shrink-0 text-sm shadow-inner border border-slate-200/60 dark:border-slate-700">
                  {index + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-slate-900 dark:text-white truncate">{lesson.title}</h4>
                  <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500 dark:text-slate-400 font-bold">
                    {lesson.type === 'quiz' ? (
                      <>
                        <span className="flex items-center gap-1.5 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-500/10 px-2 py-0.5 rounded border border-orange-100 dark:border-orange-500/20 uppercase tracking-widest text-[9px]"><ListChecks size={12} /> Assessment</span>
                        <span>{lesson.questions?.length || 0} Questions</span>
                      </>
                    ) : (
                      <>
                        <span className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-100 dark:border-indigo-500/20 uppercase tracking-widest text-[9px]"><Video size={12} /> Video</span>
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  {/* 🔥 EDIT BUTTON: Now routed to an Edit Lesson page! */}
                  <Link 
                    to={`/tutor/course/${course._id}/edit-lesson/${lesson._id}`} 
                    className="p-2.5 bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-500/30 border border-slate-200 dark:border-slate-700 rounded-xl transition-all shadow-sm" 
                    title="Edit Content"
                  >
                    <Edit2 size={16} />
                  </Link>

                  <button onClick={() => handleDeleteLesson(lesson._id, lesson.title)} className="p-2.5 bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:text-white hover:bg-rose-500 dark:hover:bg-rose-600 border border-slate-200 dark:border-slate-700 hover:border-rose-500 dark:hover:border-rose-600 rounded-xl transition-all shadow-sm" title="Delete Lesson">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
      ...
      {isCommentsOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-slate-200 dark:border-slate-800">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-slate-50/50 dark:bg-slate-950/50">
              {loadingComments ? (
                 ...
                 <div className="text-center py-20 text-slate-400 dark:text-slate-500 font-bold text-sm bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 border-dashed">No questions asked yet.</div>
              ) : (
                comments.map(c => (
                  <div key={c._id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-xs font-black text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20">{c.user?.name?.charAt(0)}</div>
            ...
            <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
               <div className="relative flex items-center shadow-sm">
                 <input type="text" placeholder="Type a reply..." className="w-full pl-5 pr-14 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-medium text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder-slate-400 dark:placeholder-slate-500" />
                 <button className="absolute right-2 p-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-colors shadow-md shadow-indigo-600/20"><Send size={16} /></button>
               </div>
            </div>
          </div>
        </div>
...
const StatWidget = ({ icon, label, value, color }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
        <div className={`p-3.5 rounded-2xl ring-1 transition-transform group-hover:scale-110 shadow-inner ${colors[color]}`}>{icon}</div>
        <div>
           <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{label}</span>
           <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none mt-1">{value}</p>
        </div>
    </div>
  );
};
