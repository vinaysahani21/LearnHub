  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800/50 pb-6 mb-8">
        ...
        <p className="text-slate-500 font-medium mt-1">Configure your local workspace environment and notifications.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800 p-8 shadow-xl shadow-slate-200/10 dark:shadow-none transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <button 
            ...
            className={`relative p-6 rounded-2xl border-2 text-left transition-all duration-300 group overflow-hidden ${!isDarkMode ? 'border-indigo-600 bg-indigo-50/50 shadow-md' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-300 bg-white dark:bg-slate-800/50'}`}
          >
            {!isDarkMode && <CheckCircle2 className="absolute top-4 right-4 text-indigo-600" size={18} />}
            <div className={`p-4 rounded-full w-fit mb-4 transition-colors ${!isDarkMode ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-500'}`}>
               <Sun size={24} />
            </div>
          ...
          >
            ...
            <p className="text-xs font-bold text-slate-500 mt-1">Cinematic studio feel</p>
          </button>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800 p-8 shadow-sm transition-all">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className="p-4 bg-orange-50 dark:bg-orange-500/10 text-orange-500 rounded-2xl">
              <Bell size={24} />
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Push Notifications</h2>
              <p className="text-sm font-medium text-slate-500 mt-1">Receive alerts for new student enrollments and cleared payouts.</p>
            </div>
          </div>
          ...
          <button 
            ...
            className={`w-16 h-8 rounded-full p-1 transition-all duration-300 ease-in-out flex items-center ${notificationsEnabled ? 'bg-emerald-500 justify-end ring-4 ring-emerald-500/20' : 'bg-slate-200 dark:bg-slate-700 justify-start'}`}
          >
